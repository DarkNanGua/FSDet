...
def parse_opt():
    parser = argparse.ArgumentParser()
                                            # 数据集
    parser.add_argument('--data', type=str, default= 'ultralytics/cfg/datasets/AI-TODv2.yaml', help='dataset.yaml path')
    ...
    # parser.add_argument('--freeze', type=int, default= 0, help='freeze')
                                        # 保存目录
    parser.add_argument('--project', default= 'output_dir/duibi/aitodv2/v8', help='save to project/name')
    parser.add_argument('--name', default='800x800', help='save to project/name')
                                        # 训练参数
    ...
    # 添加checkpoint保存间隔参数
    parser.add_argument('--save_period', type=int, default=5, help='Save checkpoint every x epochs (disabled if < 1)')
...
def disable_bn_tracking(module):
    """递归禁用所有BatchNorm2d的running stats跟踪"""
    for child in module.children():
        if isinstance(child, torch.nn.BatchNorm2d):
            child.track_running_stats = False
        else:
            disable_bn_tracking(child)  # 递归调用
...
if __name__ == '__main__':
    ...
    args = {
        ...
        "imgsz": opt.imgsz, 
        "seed": opt.seed,
        "save_period": opt.save_period,  # 添加checkpoint保存间隔
    }
    model_conf = ROOT + opt.config
    model_pretain = ROOT + opt.premodel
    model = None
    if opt.resume == True:
        model = YOLO(model_pretain)
    elif opt.premodel != '':
        model = YOLO(model_conf).load(model_pretain)
    else:
        model = YOLO(model_conf)

    disable_bn_tracking(model)  # model是你的PyTorch模型
    model.train(**args)
    # task_type.get(task)