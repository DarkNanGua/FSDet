...
from .conv import Conv as BaseConv
import math
...
class YOLOPAFPN(nn.Module):
    def __init__(self, depth = 1.0, width = 1.0, in_features = ("dark2","dark3", "dark4", "dark5"), in_channels = [128,256, 512, 1024], depthwise = False, act = "silu"):
        ...
        self.C3_p2 =  nn.ModuleList([ConvSSM1(
                hidden_dim=int(128),
                drop_path=0.,
                norm_layer=norm_layer,
                attn_drop_rate=0.,
        ...
        self.bu_conv3       = Conv(128,64, 3, 2, act=act)
        self.bu_conv2       = Conv(128,128, 3, 2, act=act)
        self.reduce_conv11 = Conv(256,64, 1, 1, act=act)
        self.out_conv1 = Conv(128,64, 3, 2, act=act)
        self.out_conv2 = Conv(128,128, 3, 2, act=act)
        self.out_conv3 = Conv(256,256, 3, 2, act=act)
        self.C3_n2 =  nn.ModuleList([ConvSSM1(
                hidden_dim=int(128),
                drop_path=0.,
                norm_layer=norm_layer,
                attn_drop_rate=0.,
        ...
        self.process0 = FeatureProcess(64)
        self.process1 = FeatureProcess(128)
        self.process2 = FeatureProcess(256)
        
    def forward(self, input):
        out_features            = self.backbone.forward(input)
        [feat0,feat1, feat2, feat3]   = [out_features[f] for f in self.in_features]   #[2, 64, 160, 160]  [2, 128, 80, 80] [2, 256, 40, 40] [2, 512, 20, 20]
        #-------------------------------------------#
        P5          = self.lateral_conv0(feat3)    # torch.Size([2, 256, 10, 10])
        #-------------------------------------------#
        P5_upsample = self.upsample(P5)           #  torch.Size([2, 256, 20, 20])
        #-------------------------------------------#
        # feat2 = self.process2(feat2,P5_upsample)
        ...
        #-------------------------------------------#
        P5_upsample=P5_upsample.permute(0,3,1,2)  #c3_p4
        
        P4          = self.reduce_conv1(P5_upsample)  # torch.Size([8, 128, 20, 20])
        P4_upsample = self.upsample(P4)    #          torch.Size([8, 128, 40, 40])
        #-------------------------------------------#
        # feat1 = self.process1(feat1,P4_upsample)
        P4_upsample = torch.cat([P4_upsample, feat1], 1)  #
        P4_upsample=P4_upsample.permute(0,2,3,1)  # torch.Size([8, 40, 40, 256])
        for blk in self.C3_p3:
            P3_out = blk(P4_upsample)       # torch.Size([8, 40, 40, 128])
        P3_out1=P3_out.permute(0,3,1,2) # c3_p3
        
        # add 高分辨率特征图
        P3          =  self.reduce_conv11(P3_out1)
        # P3          =  P3_out1
        P3_upsample = self.upsample(P3)    #          torch.Size([8, 64, 80, 80])
        #-------------------------------------------#
        # feat0 = self.process0(feat0,P3_upsample)
        P3_upsample = torch.cat([P3_upsample, feat0], 1)  #
        P3_upsample=P3_upsample.permute(0,2,3,1)  # torch.Size([8, 80, 80, 128])
        for blk in self.C3_p2:
            P2_out = blk(P3_upsample)       # torch.Size([8, 80, 80, 64])
        P2_out1=P2_out.permute(0,3,1,2) # c3_p2
        P2_out=self.out_conv1(P2_out1)
        
        
        # PAN 层
        P2_downsample   = self.bu_conv3(P2_out1)
        P2_downsample   = torch.cat([P2_downsample, P3], 1) # torch.Size([8, 256, 20, 20]))
        P2_downsample=P2_downsample.permute(0,2,3,1)  #
        for blk4 in self.C3_n2:
            P3_out = blk4(P2_downsample)
        P3_out1=P3_out.permute(0,3,1,2)   #torch.Size([8, 256, 20, 20]) 
        # P3_out=self.out_conv2(P3_out1)
        ...
        #-------------------------------------------#
        P4_out1=P4_out.permute(0,3,1,2)   #torch.Size([8, 256, 20, 20])        
        # P4_out=self.out_conv3(P4_out1)
        ...
        return (P2_out,P3_out1, P4_out1)  
      
      
      
class YOLO8PAFPN(nn.Module):
    def __init__(self, depth = 1.0, width = 1.0, in_features = ("dark2","dark3", "dark4", "dark5"), in_channels = [128,256, 512, 1024], depthwise = False, act = "silu"):
        super().__init__()
        Conv                = DWConv if depthwise else BaseConv
        self.backbone       = YOLO8Backbone(depth, width, depthwise = depthwise, act = act)
        self.in_features    = in_features
        self.upsample       = nn.Upsample(scale_factor=2, mode="nearest")
        norm_layer=nn.LayerNorm
        self.lateral_conv0  = BaseConv(int(in_channels[3] * width), int(in_channels[2] * width), 1, 1, act=act)
    ...
    def forward(self, input):
        out_features            = self.backbone.forward(input)
        [feat0,feat1, feat2, feat3]   = [out_features[f] for f in self.in_features]   #torch.Size([8, 128, 40, 40]) torch.Size([8, 256, 20, 20]) torch.Size([8, 512, 10, 10])
        #-------------------------------------------#
        P5          = self.lateral_conv0(feat3)    # torch.Size([2, 256, 10, 10])
        #-------------------------------------------#
        P5_upsample = self.upsample(P5)           #  torch.Size([2, 256, 20, 20])
...
class YoloBody(nn.Module):
    def __init__(self, num_classes, phi):
        ...
        self.backbone   = CSPDarknet(depth, width, depthwise=depthwise) #.to(device)
